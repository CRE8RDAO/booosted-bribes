...
const cre8rPrice = 0.009527; //@dunks update cre8r price
const totalBeetsEmissionsToFarms = 632863 //check if month emissions is correct: https://docs.google.com/spreadsheets/d/1zFzIpptDMebyqoPgtPNCNFEvsZcpK_6GtGEOfFOkDKU/edit#gid=386276691
const beetsPrice = 0.075;
const basicBribeForOnePercent = totalBeetsEmissionsToFarms * beetsPrice / 100;  //@dunks check basicbribe price
...
main(beetsBlockRounds[beetsBlockRounds.length-2], beetsBlockRounds[beetsBlockRounds.length-1], proposalIds[proposalIds.length-1], poolChoiceName, undefined, cre8rPrice, basicBribeForOnePercent, hasBonanza[hasBonanza.length - 1]).then(() => {
  process.exit(0);
}).catch((error) => {
  console.error(error);
  process.exit(0);
