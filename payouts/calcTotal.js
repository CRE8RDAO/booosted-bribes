...
const payoutsForAmplifi = [
  ...
  require("./out/bribe-payouts-46563257.json"),
];
...
function calcTotal() {
  ...
  for (let i = 0; i < payoutsForAmplifi.length; i++) {
    ...
    for (let j = 0; j < payouts.length; j++) {
      ...
      addressToTotalPayout[address].AMPinUSD +=
        singlePayout.boostedBonus2AmpInUSD + singlePayout.basicBoost2AmpInUSD;
  ...
  console.log(res);
  writeJSON(res, `beets-boosted-bribes-total-${Date.now().toString()}.json`);
  // todo: upload to ipfs
}
